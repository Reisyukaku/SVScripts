...
class C722F6EF1C80804DB {
    ...
    public static function QuaternionToString(quat:Quaternion):String {
        var vec = quat.f643B5D6F();
    
        var x:String = main.util.Math.S28102DAA82AC58FD((vec[0] / (2 * Math.PI)) * 360.0, 2);
        var y:String = main.util.Math.S28102DAA82AC58FD((vec[1] / (2 * Math.PI)) * 360.0, 2);
        var z:String = main.util.Math.S28102DAA82AC58FD((vec[2] / (2 * Math.PI)) * 360.0, 2);
        return "(X " + x + " , Y " + y + " , Z " + z + ")";
    }
}